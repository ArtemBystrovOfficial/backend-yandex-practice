...
enum class Direction : uint8_t { NORTH, WEST, SOUTH, EAST };
...
class Dog {
   public:
    ...
    Dog(Id id, std::string_view name, PointF position)
        : id_(id), name_(name.data(), name.size()), position_(position), speed_({0.0f, 0.0f}), direction_(Direction::NORTH){};
