...
std::shared_ptr<GameSession> Game::AddSession(Map::Id id) {
    auto map = FindMap(id);
    if (map) {
        auto session = std::make_shared<GameSession>(std::move(map), dog_speed_default_);
...
GameSession::GameSession(std::shared_ptr<Map> map, Real default_speed) : map_(std::move(map)), _last_dog_id(0), default_speed_(default_speed) {}
...
std::shared_ptr<Dog> GameSession::AddDog(std::string_view dog_name) {
    ...
    return dogs_.emplace_back(
        std::make_shared<Dog>(Dog::Id(_last_dog_id++), dog_name, PointF{(rand() % 10000) / 100.0f, (rand() % 10000) / 100.0f}, map_speed));
}
