...
std::shared_ptr<GameSession> Game::AddSession(Map::Id id) {
    auto map = FindMap(id);
    if (map) {
        auto session = std::make_shared<GameSession>(std::move(map), time_manager_, dog_speed_default_, is_game_randomize_start_cordinate_);
...
GameSession::GameSession(std::shared_ptr<Map> map, TimeManager& time_manager, Real default_speed, bool is_game_randomize_start_cordinate)
    : map_(std::move(map)), _last_dog_id(0), default_speed_(default_speed), time_manager_(time_manager) ,is_game_randomize_start_cordinate_(is_game_randomize_start_cordinate) {}
...
std::shared_ptr<Dog> GameSession::AddDog(std::string_view dog_name) {
    ...
    auto ptr = dogs_.emplace_back(std::make_shared<Dog>(
        Dog::Id(_last_dog_id++), dog_name, is_game_randomize_start_cordinate_ ? map_->GetRandomCordinates() : PointF{0.0f, 0.0f}, map_speed, map_));
