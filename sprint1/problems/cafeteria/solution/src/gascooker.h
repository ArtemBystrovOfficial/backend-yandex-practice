...
class GasCooker : public std::enable_shared_from_this<GasCooker> {
   public:
    ...
    // Используется для того, чтобы занять горелку. handler будет вызван в
    // момент, когда горелка занята Этот метод можно вызывать параллельно с
    // вызовом других методов
    void UseBurner(Handler handler) {
        ...
        net::dispatch(
            strand_,
            // За счёт захвата self в лямбда-функции, время жизни GasCooker
            // будет продлено до её вызова
            [handler = std::move(handler), self = shared_from_this(),
             this]() mutable {
                assert(strand_.running_in_this_thread());
                assert(burners_in_use_ >= 0 &&
                       burners_in_use_ <= number_of_burners_);

                // Есть свободные горелки?
                if (burners_in_use_ < number_of_burners_) {
                    // Занимаем горелку
                    ++burners_in_use_;
                    // Асинхронно уведомляем обработчик о том, что горелка
                    // занята. Используется асинхронный вызов, так как handler
                    // может выполняться долго, а strand лучше освободить
