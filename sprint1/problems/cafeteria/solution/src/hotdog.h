    ...
    HotDog(int id, std::shared_ptr<Sausage> sausage,
        ...
        if (sausage_->GetCookDuration() < MIN_SAUSAGE_COOK_DURATION ||
            ...
            throw std::invalid_argument(
                "Invalid sausage cook duration: "s +
                std::to_string(
                    std::chrono::duration_cast<std::chrono::milliseconds>(
                        sausage_->GetCookDuration())
                        .count()) +
                "ms"s);
        }

        if (bread_->GetCookDuration() < MIN_BREAD_COOK_DURATION ||
            bread_->GetCookDuration() > MAX_BREAD_COOK_DURATION) {
            throw std::invalid_argument(
                "Invalid bread cook duration: "s +
                std::to_string(
                    std::chrono::duration_cast<std::chrono::milliseconds>(
                        bread_->GetCookDuration())
                        .count()) +
                "ms"s);
        }
    }
