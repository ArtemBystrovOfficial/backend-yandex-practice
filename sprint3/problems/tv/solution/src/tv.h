...
public:
     ...
     */
    void SelectChannel(int channel) noexcept(false) {
        CheckOffAndThrow();
        if(MIN_CHANNEL > channel || channel > MAX_CHANNEL)
            throw std::out_of_range("Channel out of range");
        channel_before_ = channel_;
        channel_ = channel;
    }
     ...
     * Если телевизор выключен, выбрасывает исключение std::logic_error.
     */
    void SelectLastViewedChannel() noexcept(false)  {
        SelectChannel(channel_before_);
    }

private:

    void CheckOffAndThrow() noexcept(false) {
        if(!is_turned_on_)
